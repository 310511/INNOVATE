...
<div className="bg-white/10 backdrop-blur-xl border border-slate-700/60 rounded-2xl p-8 shadow-[0_0_50px_-15px_rgba(0,0,0,0.4)] hover:shadow-[0_0_60px_-10px_rgba(0,255,255,0.1)] transition-all">
  ...
  {/* Phone Number */}
  <div className="flex flex-col">
    <label className="text-sm text-slate-300 mb-1">Phone Number</label>
    <input
      type="tel"
      className="w-full p-3 rounded-lg bg-slate-900/60 border border-slate-700 text-white placeholder-slate-500 
                 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-600 transition"
    ...
    {visibleStartups.map((s) => (
      ...
      >
        ...
        <div className="flex items-start justify-between mb-3">
          <div>
            <h4 className="font-semibold text-lg text-white">{s.org_name}</h4>
  ...
  >
    <path d="M12 2l3 7 7 3-7 3-3 7-3-7-7-3 7-3 3-7z" />
        ...
        <div className="mt-3 text-xs text-slate-400">
          <span className="block">Industry: <span className="text-slate-300">{s.industry_type}</span></span>
          <span className="block truncate">Website: <span className="text-cyan-300">{s.website_url}</span></span>
        </div>
...
{startups.length > 2 && showAll && (
  <div className="flex justify-center mt-6">
    <button
      onClick={() => setShowAll(false)}
      className="bg-slate-800 hover:bg-slate-700 text-red-300 border border-slate-700 
                 py-2 px-6 rounded-xl text-sm transition-all"
    >
      Show Less
    </button>
  ...
  className={`bg-cyan-600 hover:bg-cyan-700 text-white py-3 px-7 rounded-xl font-semibold
              shadow-md transition-all flex items-center justify-center space-x-2
